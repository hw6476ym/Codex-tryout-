...
function buildDataset() {
  ...
  years.forEach((year, yearIdx) => {
    data[year] = countryBase.map((entry, idx) => {
      ...
      const growth = 1 + (yearIdx - 1.5) * 0.035;
      const imports = Number((impBase * growth * (1 + (idx % 5) * 0.01)).toFixed(1));
      const exports = Number((expBase * growth * (1 + (idx % 7) * 0.008)).toFixed(1));
      return {
        iso3,
        name,
        imports,
        ...
        topExports: createCategorySplit(exports, idx),
        topImports: createCategorySplit(imports, idx + 2)
      };
    });
  });
  return data;
